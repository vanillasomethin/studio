...
| GPS-tagged photos | Shop-front photo (gates stage past `new`), installed-TV photo (with the install record and map pin, gates stage past `contacted`); an EXIF fix from either fills an empty map pin (`Store.lat/lng`), which is public on the Network map from `contacted` | R2 under `verification/` (`shopPhotoUrl`, `installPhotoUrl` on `Store`) | Admin password holders; the partner (own photos) |
